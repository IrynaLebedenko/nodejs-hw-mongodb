...
export const getAllContacts = async ({ page=1, perPage=10, sortBy= 'name', sortOrder=SORT_ORDER.ASC, filter={}, userId,}) => {
  ...
  let contactsQuery = Contact.find({userId}).skip(skip).limit(limit).sort({ [sortBy]: sortOrder});
     ...
     const contactsCount = await Contact.find({userId}).merge(contactsQuery).countDocuments();
